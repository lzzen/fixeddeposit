...
///////////////////new 2026/////////////////
$address = '0x88b39402412504ab50999d13a26dc9133eac43a4';
...
//@see https://ethplorer.io/zh/
$url = "https://api.etherscan.io/v2/api?chainid=42161&module=account&action=tokenbalance&contractaddress=0xaf88d065e77c8cC2239327C5EDb3A432268e5831&address={$address}&tag=latest&apikey=PZJEDUF4JTPHJP42UD2STHEK1YP8J25PK6";
$resp = http_get($url, $header);
$usdcBalance = json_decode($resp)->result/1000000;
// print_r($resp);
...
function rendEthRow($eth, $name='ETH'){
    ...
    '操作' => '<a href="https://bscscan.com/address/0x88b39402412504ab50999d13a26dc9133eac43a4#readContract" target="btcbank">查看合约</a>',
    ];
    return $row;
}
// var_dump($usdcBalance);
function rendUSDCRow($usdcBalance=0){
    //{"status":"1","message":"OK","result":"100000000000000000000"}
    // $balance = substr($eth['rawBalance'], 0, 6) * pow(10,(strlen($eth['rawBalance'])-6-$token['tokenInfo']['decimals']));
    $balance = $usdcBalance;
    $value = $usdcBalance * 1;
    $tkaddr= '0xaf88d065e77c8cC2239327C5EDb3A432268e5831';
    $row = [
    '合约地址' => $tkaddr,
    '币种' => 'USDC',
    '余额' => number_format($balance, 6),
    '市值(USD)' => sprintf('%.02f', $value),
    '提取时间' => '<button data-token="' . $tkaddr . '" class="btn_query_tiqu">QUERY</button> <span></span>',
    '本站提取' => '<button data-token="' . $tkaddr . '" class="btn_query_tiqu_2">提现</button> <span></span>',
    '操作' => '<a href="https://arbiscan.io/address/0x88b39402412504ab50999d13a26dc9133eac43a4#readContract" target="btcbank">查看合约</a>',
    ];
    return $row;
}

function render($usdcBalance)
{
    $row = rendUSDCRow($usdcBalance);
